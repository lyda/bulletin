C
C  BULLETIN1.FOR, Version 12/18/90
...
C
	...
	IF (REMOTE_SET) THEN
     ...
     &	         FORM='UNFORMATTED')
	   ...
	   IF (IER.EQ.0) THEN
		 ...
		 I1 = I
	         CALL READDIR(I,IER)
		 IF (I1.EQ.BULL_POINT.AND.I1.NE.I) THEN
	...
	ELSE IF (READ_ONLY.OR.(MERGE.AND.REMOTE_SET.EQ.0)) THEN
	   IF (READ_ONLY) THEN
...
C
	...
	DO WHILE (NUM_COPY.GT.0.AND.IER.EQ.0)
	   ...
	   IF (FROM_REMOTE.EQ.3) THEN
	      SYSTEM = 0
	      IF (EX_BTIM(1).EQ.0.AND.EX_BTIM(2).EQ.0) THEN
...
C
	...
	DO I=SBULL,EBULL
	   ...
	   IF (IER.NE.I+1.OR.(I1.EQ.SBULL.AND.I.NE.SBULL)) THEN
...
C
	...
	IF (REMOTE_SET.EQ.3) THEN
	   WRITE (INPUT,'(1X,I5,'' of '',I5,''-'',I5)')
     &				BULL_POINT,F_START,F_NBULL
	   DO WHILE (INDEX(INPUT,'- ').GT.0)
	      I = INDEX(INPUT,'- ')
	      INPUT(I+1:) = INPUT(I+2:)
	   END DO
	ELSE
