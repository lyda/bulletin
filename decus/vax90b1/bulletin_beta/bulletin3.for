C
C  BULLETIN3.FOR, Version 1/1/91
...
C
	...
	DO WHILE (1)			! Loop once every 15 minutes
     ...
     &			INDEX(OLD_TIME,' 03:').EQ.0) THEN  ! cleanup at 3 a.m.
	   ...
	   IF (SYS_TRNLNM('BULL_NEWS_SERVER',DUMMY).AND.HOUR.EQ.0) THEN
	      CALL SYS$SETAST(%VAL(0))
...
900	CALL SYS$SETAST(%VAL(0))
	...
	IF (SYS_TRNLNM('BULL_NEWS_SERVER',DUMMY).AND.
     &			.NOT.TEST_BULLCP()) CALL NEWS2BULL
