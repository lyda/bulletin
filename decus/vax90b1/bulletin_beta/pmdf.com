...
(*                            substr (fromaddr.body, 1, fromaddr.length), *)
  ...
  BEGIN (* bulletin_master *)
    ...
    END. (* bulletin_master *)
$eod 
$copy sys$input BULLETIN_MASTER.PAS_V32
$deck
%INCLUDE 'PMDF_ROOT:[SRC]ATTRIB.INC'
...
there is a small bug in it. The following instructions are for V3.1.  It will
work with V3.2, but V3.2 does have shared images that can be linked to if you
use the MAKEFILE that comes with PMDF (although you have to modify MAKEFILE in
order to correctly point to BULL.OLB). 
