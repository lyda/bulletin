...
C
	...
	IF (INDEX_COUNT.EQ.1) THEN
	  ...
	  FOLDER_Q = FOLDER_Q1
	  CALL OPEN_BULLNEWS_SHARED		 ! Get folder file
	  ...
	  DO WHILE (IER.EQ.0)			! Copy all bulletins from file
	    ...
	    END IF
	    IF (IER.EQ.0.AND.FOLDER1_NUMBER.EQ.1000) IER = 2
	    IF (IER.EQ.0) THEN
     ...
     &		   USERNAME,READ_ACCESS,-1)
	  ...
	  CALL CLOSE_BULLNEWS			 ! We don't need file anymore
     ...
     &		FOLDER1_DESCRIP(:MIN(TRIM(FOLDER1_DESCRIP),47))
	   END IF
	  END DO
	  WRITE (6,1060)
	  FOLDER_Q = FOLDER_Q1			! Init queue pointer to header
...
1025	FORMAT (' Name',69X,'Count'/)
1030	FORMAT (1X,A,1X,I5,1X,A)
1035	FORMAT (1X,A,1X,I5)
