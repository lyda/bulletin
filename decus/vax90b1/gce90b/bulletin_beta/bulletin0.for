C
C  BULLETIN0.FOR, Version 12/16/90
...
C
	...
	DO FBULL = SBULL,EBULL
	   ...
	   IF (IER.NE.FBULL+1.OR.(FBULL1.EQ.SBULL.AND.FBULL.NE.SBULL)) THEN
...
C
	...
	IF (REMOTE_SET) THEN
	   ...
	   ALL_DIR = ALL_DIR1
	   CALL REMOTE_DIRECTORY_COMMAND(START,NBULL,REVERSE,ALL_DIR,IER)
	   IF (IER.NE.0) THEN
	      CALL CLOSE_BULLDIR
	      CALL DISCONNECT_REMOTE
	      GO TO 9999
