C
C  BULLETIN4.FOR, Version 2/26/93
...
Cb
C  FUNCTION:
C	To show privileges necessary for managing bulletin board.O
C-

	IMPLICIT INTEGER (A-Z)t

	INCLUDE 'BULLUSER.INC'K
	...
	CALL OPEN_BULLUSER_SHARED		! Get BULLUSER.DAT filea

	CALL READ_USER_FILE_HEADER(IER)

	IF (IER.EQ.0) THEN			! If header is present, exit
	   IF (NEW_FLAG(1).EQ.-1.AND.NEW_FLAG(2).EQ.-1) THEN  ! Info not presentN
	      CALL CLOSE_BULLUSER
	      CALL OPEN_BULLUSER			! Get BULLUSER.DAT file_
	      CALL READ_USER_FILE_HEADER(IER)
     ...
     & commands:'')')
	   DO I=0,38
	      IF ((I.LT.32.AND.BTEST(USERPRIV(1),I)).OR.E
     &		  (I.GT.31.AND.BTEST(USERPRIV(2),I-32))) THENN
		 WRITE (6,'(1X,A)') PRIVS(I)
	      END IFE
	   END DO
	ELSEI
	   WRITE (6,'('' ERROR: Cannot show privileges.'')')P
	END IF:

	CALL CLOSE_BULLUSER			! All finished with BULLUSERN

	CALL CHKACL(BULLUSER_FILE(:TRIM(BULLUSER_FILE)),IER)2
	IF (IER.NE.(SS$_ACLEMPTY.OR.SS$_NORMAL).AND.IER) THEN
	   CALL SHOWACL(BULLUSER_FILE(:TRIM(BULLUSER_FILE)))T
	END IF 
...
C 
	...
	DATA PRIVS 
     ...
     &	'ALTPRI','SETPRV','TMPMBX','WORLD','MOUNT','OPER','EXQUOTA',L
	...
	IF (.NOT.SETPRV_PRIV().OR..NOT.BTEST(PROCPRIV(1),PRV$V_SETPRV)) THEN1
	...
	IF (CLI$PRESENT('ID').OR.
     ...
     &	       .NE.%LOC(CLI$_ABSENT))		! Get the IDsR
	...
	DO WHILE (CLI$GET_VALUE('PRIVILEGES',INPUT_PRIV,PLEN)
     &	    .NE.%LOC(CLI$_ABSENT))		! Get the privilegesL
	   PRIV_FOUND = -1P
	   I = 0(
     ...
     &		A)') INPUT_PRIV(:PLEN)
	      RETURNB
	   ELSE IF (INPUT_PRIV(:2).EQ.'NO') THENT
	      IF (INPUT_PRIV.EQ.'NOSETPRV') THEN 
	       WRITE(6,'('' ERROR: Cannot remove SETPRV privileges.'')')T
	       RETURN
		 ...
		 OFFPRIV(2) = IBSET(OFFPRIV(2),PRIV_FOUND-32)T
	      END IFU
	   ELSE
	      IF (PRIV_FOUND.LT.32) THENM
		 ONPRIV(1) = IBSET(ONPRIV(1),PRIV_FOUND)
	      ELSE(
		 ONPRIV(2) = IBSET(ONPRIV(2),PRIV_FOUND-32)R
	      END IFL
	   END IF
	END DO
	...
	IF (IER.EQ.0) THEN			! If header is present, exit
	   ...
	   USERPRIV(1) = USERPRIV(1).AND.(.NOT.OFFPRIV(1))E
	   USERPRIV(2) = USERPRIV(2).AND.(.NOT.OFFPRIV(2))M
	   REWRITE (4) USER_HEADER_
...
C	ACCESS - Character string containing access controls to give to ID.E
...
C 
	...
	IER = SYS$PARSE_ACL('(IDENTIFIER='//ID//',ACCESS='N
     &	   //ACCESS//')',ACLENT,,) 
	IF (.NOT.IER) THENT
     ...
     &				INDEX(ACCESS,'C').EQ.0) THEN
		 ...
		 WRITE (6,'(
     &		    '' ERROR: Specified username cannot be verified.'')')U
		 CALL SYS_GETMSG(IER)t
	         RETURN
	      END IF
	      IDENT = USER + ISHFT(GROUP,16)A
	...
	IF (INDEX(ACCESS,'C').GT.0.AND.INDEX(ACCESS,'W').EQ.0) THEN
	   IER = SYS$CHANGE_ACL(,ACL$C_FILE,BULLUSER_FILE(:TRIM(P
     &		   BULLUSER_FILE)),%VAL(ACL_ITMLST),,,) 
	   RETURN
	END IFL

	IF (INDEX(FOLDER1,'.').GT.0) THEN
  	   IER = SYS$CHANGE_ACL(,ACL$C_FILE,NEWS_ACCESS(FOLDER1_DESCRIP)N
     &	      ,%VAL(ACL_ITMLST),,,)
        ELSE
  	   IER = SYS$CHANGE_ACL(,ACL$C_FILE,FOLDER1_FILEN
     &	      (:TRIM(FOLDER1_FILE))//'.BULLFIL',%VAL(ACL_ITMLST),,,)F
	END IFF
...
CO
	...
	ELSEr
	   CALL INIT_ITMLST	! Initialize item list	
	   CALL ADD_2_ITMLST(255,ACL$C_DELETEACL,%LOC(ACLENT))g
	   CALL END_ITMLST(ACL_ITMLST)	! Get address of itemlistN
	END IFZ

	IF (INDEX(ACCESS,'C').GT.0) THENO
	   IER = SYS$CHANGE_ACL(,ACL$C_FILE,BULLUSER_FILE(:TRIM(M
     &		   BULLUSER_FILE)),%VAL(ACL_ITMLST),,,)M
	   RETURN
	END IFA

	IF (INDEX(FOLDER1,'.').GT.0) THEN
  	   IER = SYS$CHANGE_ACL(,ACL$C_FILE,NEWS_ACCESS(FOLDER1_DESCRIP)_
     &	      ,%VAL(ACL_ITMLST),,,)
        ELSE
	...
	SUBROUTINE CREATE_FOLDERn
C
C  SUBROUTINE CREATE_FOLDER.
CI
C  FUNCTION: Creates a new bulletin folder.C
CC
	...
	COMMON /BULL_CUSTOM/ BULL_CUSTOMM
	...
	IF (.NOT.SETPRV_PRIV().AND.(CLI$PRESENT('ALWAYS').OR.
     &	     CLI$PRESENT('NOTIFY').OR.CLI$PRESENT('READNEW').OR.s
	...
	DO WHILE (LENDES.EQ.0))
	   IF (CLI$PRESENT('DESCRIPTION')) THEN		! DESCRIPTION specified?
	      IER = CLI$GET_VALUE('DESCRIPTION',FOLDER_DESCRIP,LENDES)E
	   ELSE
	   ...
	   END IF
	   IF (LENDES.LE.0) THEN=
	      WRITE (6,'('' Aborting folder creation.'')')P
	      RETURNE
	   ELSE IF (LENDES.GT.80) THEN		! If too many characters+
	      WRITE(6,'('' ERROR: folder must be < 80 characters.'')')T
	      LENDES = 0O
	   END IF
	END DOl

	CALL OPEN_BULLFOLDER		! Open folder filex
	...
	IF (CLI$PRESENT('OWNER')) THEN
	   IF (.NOT.SETPRV_PRIV().AND..NOT.CLI$PRESENT('ID')) THENN
     ...
     &		        '' no more than 12 characters long.'')')
		 ...
		 END IFE
	      ELSEI
	         CALL GET_UAF
     &		   (FOLDER1_OWNER,USERB1,GROUPB1,ACCOUNTB1,FLAGS,IER)U
	         IF (.NOT.IER) THEN
	...
	CALL SYS$SETDFPROT('FF00'X,CUR_DEF_PROT))
			! Set protection to (SYSTEM:RWED,OWNER:RWED,WORLD,GROUP)I

CS
C  Folder file is placed in the directory FOLDER_DIRECTORY.I
C  The file prefix is the name of the folder.
C 

	FD_LEN = TRIM(FOLDER_DIRECTORY)
	IF (FD_LEN.EQ.0) THEN
	 WRITE (6,'('' ERROR: System programmer has disabled folders.'')')E
	 GO TO 910=
	ELSEL
	...
	IF (IER.NE.0) THENT
	   WRITE(6,'('' ERROR: Cannot create folder directory file.'')')d
	   CALL ERRSNS(IDUMMY,IER)S
	   CALL SYS_GETMSG(IER)
	   GO TO 910(
	END IF	
	...
	IF (IER.NE.0) THENN
	   WRITE(6,'('' ERROR: Cannot create folder message file.'')')E
	...
	IF (CLI$PRESENT('PRIVATE').OR.CLI$PRESENT('SEMIPRIVATE')) THEN
     ...
     1	    //'.BULLDIR',STATUS='OLD',IOSTAT=IER1)o
	   OPEN (UNIT=1,FILE=FOLDER_FILE(:TRIM(FOLDER_FILE))0
	...
	ELSE
	   ...
	   IF (FOLDER1.NE.FOLDER) THEN	! Different remote folder name?o
	      REMOTE_SET = .FALSE.H
    	      CALL OPEN_BULLDIR		! If so, store name in directory fileN
	      BULLDIR_HEADER(13:) = FOLDER1
	      CALL WRITEDIR_NOCONV(0,IER)
	      CALL CLOSE_BULLDIRr
	      FOLDER1_BBOARD = FOLDER1_BBOARD(:LEN_B+2)//'*'t
	      FOLDER1 = FOLDER 
	   END IF
	   REMOTE_SET = .TRUE.l
	   IF (BTEST(FOLDER1_FLAG,0)) FOLDER_FLAG = IBSET(FOLDER_FLAG,0)N
	...
	IF (CLI$PRESENT('ADD_ONLY')) FOLDER_FLAG = IBSET(FOLDER_FLAG,11)n
	IF (CLI$PRESENT('COMPRESS')) FOLDER_FLAG = IBSET(FOLDER_FLAG,12)h

	CALL WRITE_FOLDER_FILE(IER)
	CALL MODIFY_SYSTEM_LIST(0)T
...
910	WRITE (6,'('' Aborting folder creation.'')')
	IF (FOLDER_NUMBER.EQ.0) FOLDER_SET = .FALSE.E
	CLOSE (UNIT=1,STATUS='DELETE')/
	CLOSE (UNIT=2,STATUS='DELETE')U
...
Cr
	...
	CHKPRO = SYS$PARSE_ACL('(IDENTIFIER='//INPUT(:TRIM(INPUT))//i
     &		',ACCESS=R)',ACL,,)	! Convert to ACL into binary formatx
	IF (.NOT.CHKPRO) RETURN		! Exit if can't 

	FLAGS = CHP$M_READ		! Specify read access checkingE
	...
	CHKPRO = SYS$CHKPRO(%VAL(ACL_ITMLST))	! Check if process has theL
...
C 
	...
	IF (CLI$PRESENT('FILESPEC')) THEN
     ...
     &		READONLY,SHARED,ERR=920,FORM='FORMATTED')L
	   CALL ENABLE_PRIVS	! Reset SYSPRV privilegesD
	END IFT
C.
C  If file specified in command, read file.m
C  Else, read  from the terminal./
C8

	IF (EDITIT) THEN			! If /EDIT specified
	   IF (LEN_P.EQ.0) THEN			! If no file param specifiedN
	      CALL MAILEDIT('SYS$LOGIN:BULL.SCR',' ')
	      OPEN (UNIT=3,FILE='SYS$LOGIN:BULL.SCR',STATUS='OLD',P
     &		 DISPOSE='DELETE',ERR=910,FORM='FORMATTED')D
	      LEN_P = 1
	   ELSE
	      CLOSE (UNIT=3) 
	      CALL MAILEDIT(BULL_PARAMETER(:LEN_P),'SYS$LOGIN:BULL.SCR')I
	      OPEN (UNIT=3,FILE='SYS$LOGIN:BULL.SCR',STATUS='OLD',N
     &		 DISPOSE='DELETE',ERR=910,FORM='FORMATTED')T
	   END IF
...
1000	   FORMAT (' Enter newsgroup description:',
     ...
     &			''.  Reinput:'')') LINE_LENGTHg
	      ELSE IF (ILEN.GE.0) THEN		! If good input line entered_
...
2010	         FORMAT(A)E
	...
	CALL NEWS_POST('newgroup '//FOLDER1_NAME(:LEN_F),.TRUE.,IER,A
     &		       'Adding newsgroup.')R
	CLOSE (UNIT=3)N
...
910	WRITE(6,1010)_
1010	FORMAT (' No news group was added.')A
	CLOSE (UNIT=3)(
	RETURN,
	...
	ENTRY COMPRESS(IN,OUT,O)T

	L = LEN(IN)
	O = 1
	K = 1
	DO WHILE (K.LT.L)
	   T(O:O) = A(ICHAR(IN(K:)).AND.127,ICHAR(IN(K+1:)).AND.127)A
	   IF (T(O:O).NE.' ') THENa
	      K = K + 2
	      O = O + 1
	   ELSE IF (L.GT.K+2.AND.IN(K:K).EQ.IN(K+1:K+1).AND.i
     &		    IN(K:K+1).EQ.IN(K+2:K+3)) THEN
	      C = 4
	      K = K + 4
	      DO WHILE (K.LE.L.AND.IN(K:K).EQ.IN(K-1:K-1))K
		 C = C + 1
		 K = K + 1
	      END DO_
	      T(O:O+2) = CHAR(255)//CHAR(C)//IN(K-1:K-1)K
	      O = O + 3
