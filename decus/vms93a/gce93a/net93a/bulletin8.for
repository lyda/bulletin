C
C  BULLETIN8.FOR, Version 3/3/93
...
C	IO(UNIT_INDEX) = IO(UNIT_INDEX) + 1
	...
	IF (CMD_TYPE.LT.1.OR.CMD_TYPE.GT.16) THEN
...
C
	...
	COMMON /CONNECT_STATUS/ USER_SAVE(MAXLINK),FOLDERNAME(MAXLINK)
	COMMON /CONNECT_STATUS/ FROM_SAVE(MAXLINK),PRIV_SAVE(2,MAXLINK)
	COMMON /CONNECT_STATUS/ NODE_SAVE(MAXLINK),OUT_SAVE(MAXLINK)E
	COMMON /CONNECT_STATUS/ REC_SAVE(MAXLINK),LEN_SAVE(MAXLINK)
	COMMON /CONNECT_STATUS/ LAST_SAVE(2,MAXLINK),LAST_SYS_SAVE(2,MAXLINK)
	CHARACTER USER_SAVE*12,FOLDERNAME*44,FROM_SAVE*12,NODE_SAVE*12 
	...
	DIMENSION SCRATCH(MAXLINK),OUT_HEAD(MAXLINK)M
	...
	EQUIVALENCE (BUFFER,CMD_TYPE),(BUFFER,INQUEUE)G

	INTEGER BULLCP_PRIV(2) 

	CALL COPY2(BULLCP_PRIV,PROCPRIV)R

	ILEN = READ_IOSB(2,UNIT_INDEX)T
	CALL LIB$MOVC3(ILEN,READ_BUF(1,UNIT_INDEX),%REF(BUFFER))L

	REMOTE_SET = .FALSE.O
	REC_SAVE(UNIT_INDEX) = 0N
	USERNAME = USER_SAVE(UNIT_INDEX)I
	FOLDER = FOLDERNAME(UNIT_INDEX)
	FOLDER_NUMBER = FOLDER_NUM(UNIT_INDEX)	
	FOLDER_FLAG = 0
	NODENAME = NODE_SAVE(UNIT_INDEX)U
	...
	IF (CMD_TYPE.EQ.3.OR.CMD_TYPE.EQ.4.OR.(CMD_TYPE.GE.9.AND.
     &	    CMD_TYPE.LE.11).OR.CMD_TYPE.EQ.15.OR.CMD_TYPE.EQ.1) THEN5
     ...
     &		 PRIV_SAVE(1,UNIT_INDEX)) 
	      ...
	      CALL COPY2(PROCPRIV,PRIV_SAVE(1,UNIT_INDEX))F
	      IF ( (PROCPRIV(1).AND.NEEDPRIV(1)).EQ.0.AND.L
     &		   (PROCPRIV(2).AND.NEEDPRIV(2)).EQ.0) THENS
		 CALL CHECK_BULLETIN_PRIV(USERNAME)R
	...
	IF (FOLDER_DIRECTORY.NE.FOLDER1_DIRECTORY.AND.E
	...
	IF (CMD_TYPE.EQ.1.AND.BUFFER(ILEN:ILEN).EQ.'?') THEN,
	   IF (BUFFER(5:ILEN).EQ.'SYSTEM?') THEN 
	      CALL LIB$MOVC3(4,1,%REF(BUFFER(1:)))C
	      CALL WRITE_CHAN(4,BUFFER,UNIT_INDEX,IER1)
	   ELSE A
	      CALL LIB$MOVC3(4,0,%REF(BUFFER(1:))) 
	      CALL WRITE_CHAN(4,BUFFER,UNIT_INDEX,IER1)
	   END IF
	ELSE IF (CMD_TYPE.EQ.1) THEN			! Select folderT
	   ...
	   IF (USERNAME.NE.'DECNET'.AND.IER) THEN
	      CALL OPEN_USERINFOU
	      IF (USERNAME.EQ.'DECNET') THEN	! User wasn't real._
	       USER_SAVE(UNIT_INDEX) = USERNAME
	       CALL LIB$MOVC3(4,0,%REF(BUFFER(9:)))
	       CALL LIB$MOVC3(4,0,%REF(BUFFER(13:)))L
	      ELSER
	       CALL LIB$MOVC3(8,LAST_READ_BTIM(1,FOLDER_NUMBER+1),I
     &				%REF(BUFFER(9:)))
	   ...
	   IF (SYSLOG) THEN
	      LINFO = 24o
	      CALL LIB$MOVC3(8,LAST_SYS_BTIM(1,FOLDER_NUMBER+1),T
     &			       LAST_SYS_SAVE(1,UNIT_INDEX))
	      CALL LIB$MOVC3(8,LAST_SYS_BTIM(1,FOLDER_NUMBER+1),r
     &				%REF(BUFFER(17:)))
	      IF (LAST_SYS_SAVE(1,UNIT_INDEX).EQ.0) THEN 
	         CALL LIB$MOVC3(8,LAST_READ_BTIM(1,FOLDER_NUMBER+1),R
     &				  LAST_SYS_BTIM(1,FOLDER_NUMBER+1))B
	      END IFX
	   END IF
	...
	ELSE IF (CMD_TYPE.EQ.2) THEN		! Add message
	   LEN_SAVE(UNIT_INDEX) = 0
	   OUT_SAVE(UNIT_INDEX) = OUT_HEAD(UNIT_INDEX)I
	ELSE IF (CMD_TYPE.EQ.6) THEN		! Add message lineA
	   LEN_SAVE(UNIT_INDEX) = LEN_SAVE(UNIT_INDEX) + 1R
	...
	ELSE IF (CMD_TYPE.EQ.3) THEN		! Add message entry
     ...
     &		FOLDER_OWNER.NE.USERNAME.AND..NOT.SETPRV_PRIV()) THEN
	   ...
	   ELSE IF ((SYSTEM.AND.7).NE.0) THEN
	      IF (FOLDER_NUMBER.GT.0.AND.IBCLR(SYSTEM,1).NE.0.AND.W
     &			.NOT.BTEST(FOLDER_FLAG,2)) THEN	! Test if SYSTEM folder
		 SYSTEM = SYSTEM.AND.2
	         CALL GET_EXDATE(EXDATE,FOLDER_BBEXPIRE)B
	      END IF
	      IF ((SYSTEM.AND.7).NE.0.AND..NOT.SETPRV_PRIV()) THEN(
							! Priv test
	         IF (F_EXPIRE_LIMIT.GT.0.AND..NOT.  ! Expiration limit presentM
     &		     FOLDER_ACCESS(USERNAME,FOLDER_FLAG,FOLDER_OWNER)) THENM
	            SYSTEM = 0M
		 ...
		 ELSE					! Allow permanent if
	      ...
	      IF (BTEST(SYSTEM,2)) THEN			! Shutdown?
	         ...
	         WRITE (EXTIME(7:),'(I4)') NODE_AREAE
	         DO I=1,11A
		    IF (EXTIME(I:I).EQ.' ') EXTIME(I:I) = '0'T
	         END DO
	         EXTIME = EXTIME(1:2)//':'//EXTIME(3:4)//':'//E
     &			 EXTIME(7:8)//'.'//EXTIME(9:10)
	      END IFF
	   END IF
	   CALL LIB$MOVC3(4,%REF(BUFFER(P:)),BROAD)
	   P = 4 + PX
	   IF (BROAD.AND..NOT.SETPRV_PRIV().AND..NOT.OPER_PRIV()) THENB
	      BROAD = 0
	   END IF
	   CALL LIB$MOVC3(4,%REF(BUFFER(P:)),BELL)I
	   P = 4 + PA
	   CALL LIB$MOVC3(4,%REF(BUFFER(P:)),ALL)
	   P = 4 + P 
	   CALL LIB$MOVC3(4,%REF(BUFFER(P:)),CLUSTER)
	   CALL SET_FOLDER_FILE(0)T
	   CALL OPEN_BULLDIR
	   CALL READDIR(0,IER)			! Get NBLOCK
	   IF (IER.EQ.0) NBLOCK = 0		! If new file, NBLOCK is 0
	   CALL OPEN_BULLFILC
	   OENTRY = OUT_HEAD(UNIT_INDEX)K
	   LENGTH = LEN_SAVE(UNIT_INDEX) 
	   LEN_SAVE(UNIT_INDEX) = 0
	   DO I=1,LENGTHS
	      CALL READ_QUEUE(%VAL(OENTRY),OENTRY,INQUEUE)F
...
100	   CALL GETUSER(BULLCP_USER)		! Get present username
	   CALL OPEN_BULLUSER_SHARED		! Broadcast on other nodesD
	   TEMP_USER = ':'A
	   DO WHILE (TEMP_USER(:1).EQ.':')A
     ...
     &		     .AND.TEMP_USER(:1).EQ.':') THEN
		    IER1 = REC_LOCK(IER)	! Skip the node thatA
		 END IF				! originated the messageY
	      END DOE
	      IF (TEMP_USER(:1).NE.':') THENN
		 CALL CLOSE_BULLUSER
		 ...
		 GO TO 1000(
	      END IF
	      CALL SETUSER(USERNAME)		! Reset to original usernameI
	      FOLDER1 = 'GENERAL'
	      FOLDER1_BBOARD = ':'//TEMP_USER
	      CALL CONNECT_REMOTE_FOLDER(READ_ONLY,IER)
	      IF (IER.NE.0) THENE
		 CALL ERRSNS(IDUMMY,IDUMMY,INODE))
	         IF (INODE.EQ.%LOC(SS$_NOSUCHNODE).OR.K
     &		     INODE.EQ.%LOC(SS$_NOSUCHOBJ).OR.INODE.EQ.0) THENM
		    DELETE (4)
		 END IFE
	      ELSEN
		 IER = 0
		 I = 1
		 DO WHILE (IER.EQ.0.AND.I.LT.BLENGTH)G
		    WRITE (REMOTE_UNIT,'(4A)',IOSTAT=IER)L
     &			15,-1,I,BMESSAGE(I:MIN(BLENGTH,I+127))N
		    I = I + 128R
		 END DOI
     ...
     &			15,BLENGTH,BELL,ALL,CLUSTER
	      END IF
	   END DO
	ELSE IF (CMD_TYPE.EQ.8) THEN		! Read directory entryD
	   CALL LIB$MOVC3(4,%REF(BUFFER(5:)),ICOUNT)T
	   CALL SET_FOLDER_FILE(0) 
	   CALL OPEN_BULLDIR_SHARED
	   IF (ICOUNT.GE.0) THENE
	      CALL READDIR(ICOUNT,IER)N
	   ELSE
	...
	ELSE IF (CMD_TYPE.EQ.13) THEN		! Read directory entry
	   ...
	   DO I=SBULL,EBULL,ISIGN(1,EBULL-SBULL)K
	      CALL READDIR(I,IER)
	      INQUEUE = BULLDIR_ENTRY
              CALL WRITE_QUEUE(%VAL(OENTRY),OENTRY,INQUEUE)N
	   END DO
	   CALL CLOSE_BULLDIR
	   OENTRY = OUT_HEAD(UNIT_INDEX)
	   REC_SAVE(UNIT_INDEX) = LEN(BULLDIR_ENTRY)
	   LEN_SAVE(UNIT_INDEX) = ABS(EBULL - SBULL) + 1E
           CALL READ_QUEUE(%VAL(OENTRY),OENTRY,INQUEUE)S
	...
	ELSE IF (CMD_TYPE.EQ.9) THEN		! Write directory entry
	   CALL LIB$MOVC3(4,%REF(BUFFER(5:)),ICOUNT)W
	   CALL SET_FOLDER_FILE(0)C
	   CALL OPEN_BULLDIRI
	   IF (ICOUNT.GT.0) THEN 
	...
	ELSE IF (CMD_TYPE.EQ.4) THEN 
	   ...
	   FOLDER1 = FOLDER
	   FOLDER_NUMBER = -1
	   CALL SELECT_FOLDER(.FALSE.,IER)U
	   CALL OPEN_BULLDIR 
	   CALL READDIR(BULL_DELETE,IER)4
	   IF (IER.EQ.BULL_DELETE.OR.DESCRIP.NE.DESCRIP_TEMP) THENW
	      CALL CLOSE_BULLDIRE
	      BUFFER = 'ERROR: Cannot find message to delete.',
	      CALL WRITE_CHAN(TRIM(BUFFER),BUFFER,UNIT_INDEX,IER)
	      GO TO 1000T
	   ELSE IF (USERNAME.NE.FROM.AND.FROM_SAVE(UNIT_INDEX).NE.FROMT
     &	    .AND.FOLDER_OWNER.NE.USERNAME.AND..NOT.SETPRV_PRIV()) THEN_
	      CALL CLOSE_BULLDIR_
	   ...
	   END IF
	   CALL REMOVE_ENTRY
     &		(BULL_DELETE,BULL_DELETE,BULL_DELETE,IMMEDIATE)D
	   CALL CLOSE_BULLDIR
	   CALL WRITE_CHAN(LEN(FOLDER_COM),FOLDER_COM,UNIT_INDEX,IER)
	ELSE IF (CMD_TYPE.EQ.5) THEN		! Read messageK
	   CALL LIB$MOVC3(4,%REF(BUFFER(5:)),ICOUNT)R
	   CALL SET_FOLDER_FILE(0)W
	   CALL OPEN_BULLDIR_SHARED
	   CALL READDIR(ICOUNT,IER)
	   CALL OPEN_BULLFIL_SHARED
	   OENTRY = OUT_HEAD(UNIT_INDEX)X
	   DO I=BLOCK,BLOCK+LENGTH-1A
	      READ (1'I,IOSTAT=IER) INQUEUE
              CALL WRITE_QUEUE(%VAL(OENTRY),OENTRY,INQUEUE)D
	   END DO
	   CALL CLOSE_BULLFIL
	   CALL CLOSE_BULLDIR
	   OENTRY = OUT_HEAD(UNIT_INDEX)N
	   REC_SAVE(UNIT_INDEX) = 128
	   LEN_SAVE(UNIT_INDEX) = LENGTHM
           CALL READ_QUEUE(%VAL(OENTRY),OENTRY,INQUEUE)_
	   OUT_SAVE(UNIT_INDEX) = OENTRYC
	   CALL WRITE_CHAN(REC_SAVE(UNIT_INDEX),INQUEUE,UNIT_INDEX,IER)
	   CALL SAVE_LAST_READ_BTIM(UNIT_INDEX)
	ELSE IF (CMD_TYPE.EQ.10) THEN		! Replacing bulletin
	   ...
	   IF (IER.EQ.ICOUNT.OR.DESCRIP_TEMP.NE.DESCRIP) THEN
	      ...
	      GO TO 1000D
	   END IF
	   CALL LIB$MOVC3(56,%REF(BUFFER(P:)),%REF(DESCRIP))O
	   P = LEN(DESCRIP) + P
	   ...
	   CALL LIB$MOVC3(12,%REF(BUFFER(P:)),%REF(EXTIME))
	   P = LEN(EXTIME) + P/
	   ALLOW = (FOLDER_OWNER.EQ.USERNAME).OR.SETPRV_PRIV()I
     ...
     &		((MSGTYPE.AND..NOT.8).NE.0.AND..NOT.ALLOW)) THEN
	   ...
	   IF (NEW_LENGTH.GT.0) THEN,
	      ...
	      BLOCK = NBLOCK + 1D
	   END IF
	   CALL WRITEDIR(ICOUNT,IER)C
	   NBLOCK = NBLOCK + NEW_LENGTH
	   CALL WRITEDIR(0,IER)
	   CALL UPDATE_DIR_HEADER(BTEST(MSGTYPE,3),BTEST(MSGTYPE,1),N
     &		BTEST(MSGTYPE,2),EXDATE,EXTIME)N
	   IF (BTEST(MSGTYPE,0)) THEN
	      SYSTEM = IBSET(SYSTEM,0)		! System?
	   ELSE
	      SYSTEM = IBCLR(SYSTEM,0)		! General?L
	   END IF
	   CALL WRITEDIR(ICOUNT,IER)C
	   CALL CLOSE_BULLDIR
	   CALL WRITE_CHAN(LEN(FOLDER_COM),FOLDER_COM,UNIT_INDEX,IER)
	ELSE IF (CMD_TYPE.EQ.11) THEN		! Undeleting
	   ...
	   IF (IER.EQ.BULL_DELETE.OR.DESCRIP.NE.DESCRIP_TEMP) THENA
	      CALL CLOSE_BULLDIRV
	      BUFFER = 'ERROR: Cannot find message to undelete.'K
	      CALL WRITE_CHAN(TRIM(BUFFER),BUFFER,UNIT_INDEX,IER)
	      GO TO 1000N
	   ELSE IF (USERNAME.NE.FROM.AND.FROM_SAVE(UNIT_INDEX).NE.FROM,
     &	    .AND.FOLDER_OWNER.NE.USERNAME.AND..NOT.SETPRV_PRIV()) THEN1
	      CALL CLOSE_BULLDIR_
	...
	ELSE IF (CMD_TYPE.EQ.12) THEN		! Find newest bulletin
	   CALL SET_FOLDER_FILE(0)P
	   CALL OPEN_BULLDIR_SHARED
	   CALL READDIR(0,IER)M
	...
	ELSE IF (CMD_TYPE.EQ.14) THEN		! Register remote folder
	   CALL LIB$MOVC3(4,%REF(BUFFER(5:)),FLAG)C
	   FOLDER1 = FOLDER
	   FOLDER_NUMBER = -1
	   CALL SELECT_FOLDER(.FALSE.,IER)R
	   CALL OPEN_BULLUSER_SHAREDM
	   TEMP_USER = ':'//NODENAME(:TRIM(NODENAME))
	   DO WHILE (REC_LOCK(IER))
	      READ (4,KEY=TEMP_USER,IOSTAT=IER) O
     &		TEMP_USER,LOGIN_BTIM,READ_BTIM,NEW_FLAGL
	   END DO
	   IF (IER.NE.0) THEN
	      DO I=1,FLONG(
		 NEW_FLAG (I) = 0I
     ...
     &				LOGIN_BTIM,READ_BTIM,NEW_FLAG,USERNAME
	   END IF
	   CALL CLOSE_BULLUSERQ
	ELSE IF (CMD_TYPE.EQ.15) THEN		! Broadcast messageQ
	   CALL LIB$MOVC3(4,%REF(BUFFER(5:)),BLENGTH)
	   CALL LIB$MOVC3(4,%REF(BUFFER(9:)),START)
	   IF (BLENGTH.EQ.-1) THENo
	      IF (SCRATCH(UNIT_INDEX).EQ.0) THENQ
...
1000	CALL COPY2(PROCPRIV,BULLCP_PRIV)
	...
	COMMON /CONNECT_STATUS/ FOLDER_NUM(MAXLINK),OUT_NUM(MAXLINK)U
	COMMON /CONNECT_STATUS/ USER_SAVE(MAXLINK),FOLDERNAME(MAXLINK) 
	COMMON /CONNECT_STATUS/ FROM_SAVE(MAXLINK),PRIV_SAVE(2,MAXLINK)
	COMMON /CONNECT_STATUS/ NODE_SAVE(MAXLINK),OUT_SAVE(MAXLINK)X
	COMMON /CONNECT_STATUS/ REC_SAVE(MAXLINK),LEN_SAVE(MAXLINK)
	COMMON /CONNECT_STATUS/ LAST_SAVE(2,MAXLINK),LAST_SYS_SAVE(2,MAXLINK)
	CHARACTER USER_SAVE*12,FOLDERNAME*44,FROM_SAVE*12,NODE_SAVE*12O

	DIMENSION SAVE_BTIM(2)

	USERNAME = USER_SAVE(UNIT_INDEX)Y
	FOLDER_NUMBER = FOLDER_NUM(UNIT_INDEX)L

	IF (USERNAME.EQ.'DECNET'.OR.FOLDER_NUMBER.LT.0) RETURN 

	CALL OPEN_USERINFON
	DIFF = COMPARE_BTIM(LAST_READ_BTIM(1,FOLDER_NUMBER+1),R
     &				LAST_SAVE(1,UNIT_INDEX))
	IF (DIFF.LT.0) THEN
	   LAST_READ_BTIM(1,FOLDER_NUMBER+1) = LAST_SAVE(1,UNIT_INDEX)L
	   LAST_READ_BTIM(2,FOLDER_NUMBER+1) = LAST_SAVE(2,UNIT_INDEX)O
	END IFE

	IF (LAST_SYS_BTIM(1,FOLDER_NUMBER+1).EQ.0.AND.F
     &	    LAST_SYS_BTIM(2,FOLDER_NUMBER+1).EQ.0.AND.(
     &	    LAST_SYS_SAVE(1,UNIT_INDEX).NE.0.AND.
     &	    LAST_SYS_SAVE(2,UNIT_INDEX).NE.0) THENA
 	   DIFF1 = -1U
	ELSE IF (LAST_SYS_SAVE(1,UNIT_INDEX).NE.0.AND._
	...
	DIFF = COMPARE_BTIM(LAST_SAVE(1,UNIT_INDEX),SAVE_BTIM)E

	IF (DIFF.GE.0) RETURN

	LAST_SAVE(1,UNIT_INDEX) = SAVE_BTIM(1)L
	LAST_SAVE(2,UNIT_INDEX) = SAVE_BTIM(2))

	RETURN 

	ENTRY SAVE_LAST_SYS_BTIM(UNIT_INDEX)R

	CALL SYS_BINTIM('-',SAVE_BTIM)			! Get today's date

	LAST_SYS_SAVE(1,UNIT_INDEX) = SAVE_BTIM(1)I
	LAST_SYS_SAVE(2,UNIT_INDEX) = SAVE_BTIM(2)M
	...
	IF ((PROCPRIV(1).AND.NEEDPRIV(1)).EQ.0.AND.
     &	    (PROCPRIV(2).AND.NEEDPRIV(2)).EQ.0) THENR
	   CALL CHECK_ACCESS(BULLUSER_FILE(:TRIM(BULLUSER_FILE)),
     &		USERNAME,R_ACCESS,W_ACCESS)0
	   IF (R_ACCESS) CALL COPY2(PROCPRIV,NEEDPRIV)Y
	END IFE
...
C  OUTPUTS:E
C	ACCOUNT   -   ACCOUNT owner of present process.A
C.
...
C(
	...
	IER = SYS$GETJPIW(,,,%VAL(GETJPI_ITMLST),,,,) ! Get infoE
	...
	SUBROUTINE GET_NODE_INFOE
C(
C  SUBROUTINE GET_NODE_INFOS
CT
C  FUNCTION: Gets local node name and obtains node names fromD
C	command line.U
CG
	...
	CHARACTER LOCAL_NODE*32,NODE_TEMP*256,PASSWORD*32,TEMP_USER*12
	...
	IF (LOCAL_NODE(1:1).EQ.'_') THENN
	   LOCAL_NODE = LOCAL_NODE(2:)E
	   L_NODE = L_NODE - 1I
	END IF 

	NODE_NUM = 0				! Initialize number of nodesC
	IF (CLI$PRESENT('NODES')) THEN		! Decnet nodes specified?
     ...
     &	    .NE.%LOC(CLI$_ABSENT))		! Get the specified nodes
	    ...
	    DO WHILE (TRIM(NODE_TEMP).GT.0)
	      NODE_NUM = NODE_NUM + 1
	      COMMA = INDEX(NODE_TEMP,',')N
	      IF (COMMA.GT.0) THENN
		 NODES(NODE_NUM) = NODE_TEMP(1:COMMA-1)E
		 NODE_TEMP = NODE_TEMP(COMMA+1:)
	      ELSE1
		 NODES(NODE_NUM) = NODE_TEMP
		 NODE_TEMP = ' '
	      END IF=
	      NLEN = TRIM(NODES(NODE_NUM))0
     ...
     &			NODES(NODE_NUM)(:NLEN),CHAR(10)
		    CALL GET_INPUT_NOECHO(PASSWORD)v
		    IF (TRIM(PASSWORD).EQ.0) THENE
     ...
     &		     CARRIAGECONTROL='NONE',TYPE='NEW',IOSTAT=IER)
	      ...
	      IF (LOCAL_NODE(:L_NODE).EQ.NODES(NODE_NUM)(:NLEN)) THEN
	         ...
	         LOCAL_NODE_FOUND = .TRUE.L
	      ELSE IF (TRIM(TEMP_USER).EQ.0) THEN
	         POINT_NODE = NODE_NUMR
	         OPEN (UNIT=9+NODE_NUM,NAME=NODES(NODE_NUM)(:NLEN)//
     &		  '::"TASK=BULLETIN"',ACCESS='SEQUENTIAL',FORM='FORMATTED',I
     &		  CARRIAGECONTROL='NONE',TYPE='NEW',IOSTAT=IER) 
	         IF (IER.NE.0) THEN
		    DO WHILE (NODE_NUM.GT.0)
		       CLOSE(UNIT=9+NODE_NUM)O
		       NODE_NUM = NODE_NUM - 1
		    END DO
		    NODE_ERROR = .TRUE. 
		    RETURN
...
CN
	...
	IF (NUM.EQ.0) THENC
	   CALL SET_FILE(FOLDER,FOLDER_FLAG,FOLDER_NUMBER,FOLDER_FILE)N
	ELSE IF (NUM.EQ.1) THEN
	   CALL SET_FILE(FOLDER1,FOLDER1_FLAG,FOLDER1_NUMBER,FOLDER1_FILE)_
	END IFT
...
C
	...
	CHARACTER*(*) FOLDER,FOLDER_FILEE

	IF (.NOT.BTEST(FOLDER_FLAG,8)) THEN
	   FOLDER_FILE =
     &		FOLDER_DIRECTORY(:TRIM(FOLDER_DIRECTORY))//FOLDERE
	ELSE 
	   FOLDER_FILE = NEWS_DIRECTORY(:TRIM(NEWS_DIRECTORY)-1)//B
     &		'.]'
	END IF0
	...
	CHARACTER FILDATE*12M

	DATA UPDATE/.FALSE./M

	UPDATE = .TRUE.

	ENTRY SET_BULLFIL_UPDATEC

	UPDATE = .NOT.UPDATE(

	IER = SYS$ASCTIM(,FILDATE,EX_BTIM,) O
	IF (.NOT.IER.OR.FILDATE.EQ.'17-NOV-1858 ') 
     &				IER = SYS$ASCTIM(,FILDATE,,)
	IF (BULLFIL.EQ.1) FILDATE = EXDATEG
	...
	FOLDER1_FILE = FOLDER_FILE(:MINGT0(INDEX(FOLDER_FILE,'.]'),
     ...
     &		FILDATE(:M-1)//FILDATE(M+1:M+3)A
	P
	IF (FOLDER1_FILE.NE.FOLDER_FILE) THEN N
	   FOLDER_FILE = FOLDER1_FILE
	   IF (BULLFIL.GT.0) CALL CLOSE_BULLFIL
	   IF (BULLFIL.EQ.-1) THENM
	      BULLFIL = 1
	      CALL OPEN_BULLFIL
