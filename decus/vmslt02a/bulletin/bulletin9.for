C
C  BULLETIN9.FOR, Version 3/12/00
...
C
	...
	IF (TRIM(USERNAME).EQ.0) USE_INFROM = 1		! PMDF OR SIMILAR
...
C	            IF (.NOT.NEWS_FEED()) THEN
		       TEXT = .TRUE.
C		    ELSE
C	               CALL STRIP_HEADER(BUFFER,TRIM(BUFFER),IER)
C		       TEXT = .NOT.IER
C		    END IF
C		 END IF
		 IF (TEXT) THEN
...
C	   IF (NEWS_FEED().AND..NOT.TEXT) THEN
C	      CALL STRIP_HEADER(BUFFER,TRIM(BUFFER),IER)
C	      TEXT = .NOT.IER
C	   ELSE
	      TEXT = .TRUE.
C	   END IF
	   IF (.NOT.STORED.AND.BTEST(FOLDER_FLAG,11)) THEN
C     &	       ((TEXT.AND.NEWS_FEED()).OR.BTEST(FOLDER_FLAG,11))) THEN
	      WRITE (3,'(A)') BUFFER(:MIN(LEN_BUFFER,LINE_LENGTH))
	   END IF
	END IF
...
C	I = INDEX(INFROM,'(')
C	IF (I.GT.0.AND.INDEX(INFROM(I+1:),'@').GT.0) THEN ! Name may be of form
C	   INFROM = INFROM(INDEX(INFROM,'(')+1:)   ! personal-name (net-name)
C	END IF
