	...
	DO WHILE ((STEP_BULL.EQ.1.AND.BULL_SEARCH.LE.END_BULL).OR.
     ...
     &			BULL_SEARCH,DUMMY)
	   ...
	   IF (IER.EQ.BULL_SEARCH+1.AND.SEARCH_MODE.NE.2) THEN
	      ...
	      ELSE
		 IF (LEN(DESCRIP)-1.LE.TRIM(DESCRIP)) THEN
	            IF (REMOTE_SET) THEN
		 ...
		 END IF
	         IF (DESCRIP1(:4).EQ.'RE: ') DESCRIP1 = DESCRIP1(5:)
		 IF (TRIM(SEARCH_STRING).GE.LEN(DESCRIP)-5.AND.
     &		     TRIM(DESCRIP1).GT.TRIM(SEARCH_STRING))
     &		    DESCRIP1 = DESCRIP1(:TRIM(SEARCH_STRING))
	      END IF
     ...
     &		   DESCRIP1(:LEN(DESCRIP)).EQ.SEARCH_STRING)))) THEN
                 IF (.NOT.NEGATED) THEN
		    FOUND = BULL_SEARCH
		    GO TO 900
	         END IF   
