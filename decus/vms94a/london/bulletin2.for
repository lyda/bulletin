C
C  BULLETIN2.FOR, Version 3/11/94
...
C
	...
	IF (EDIT) THEN
	   ...
	   END IF
	   IF (IER.EQ.0) THEN
	      CALL ADD_SIGNATURE(0,'SYS$LOGIN:BULL.SCR',FOLDER_NAME)r
	      IF ((NEWS_FEED().OR.REMOTE_SET.GE.3).AND.LIST) THEN
	         CALL NEWS_POST('SYS$LOGIN:BULL.SCR',.FALSE.,IER,
     &			INDESCRIP)b
		 STATUS = IER.EQ.0
	         IF (IER.EQ.0) THEN
		    WRITE (6,'('' Message successfully posted.'')')s
	         END IF
	      END IFS
	      IF (IER.EQ.0.AND.LENFRO.GT.0) THENT
	         CALL RESPOND_MAIL('SYS$LOGIN:BULL.SCR',INFROM,
     &			   INDESCRIP,STATUS)I
	      END IFZ
	   END IF
	ELSEL
	   OPEN(UNIT=3,FILE='SYS$LOGIN:BULL.SCR',IOSTAT=IER,U
     &		RECL=LINE_LENGTH,STATUS='NEW',CARRIAGECONTROL='LIST')
	   IF (.NOT.FILESPEC) THENB
	      WRITE (6,'('' Enter message: End with ctrl-z,'',Y
     &			 '' cancel with ctrl-c'')')
	      ILEN = LINE_LENGTH + 1		! Length of input lineE
	      ICOUNT = 0			! Character count counterA
	      DO WHILE (ILEN.GE.0)		! Input until no more input
	         CALL GET_LINE(INPUT,ILEN)	! Get input line
	         IF (ILEN.GT.LINE_LENGTH) THEN	! Input line too longN
		    WRITE(6,'('' ERROR: Input line length > '',I,O
     &			   ''.  Reinput:'')') LINE_LENGTH
	         ELSE IF (ILEN.GE.0) THEN	! If good input line enteredN
	   ...
	   ELSE
	      ...
	      ICOUNT = 0	
	      DO WHILE (IER.EQ.0)
		 READ (4,'(Q,A)',IOSTAT=IER) ILEN,INPUTI
		 IF (IER.EQ.0) THEN
		    ICOUNT = ICOUNT + 1'
		    WRITE (3,'(A)') INPUT(:ILEN)
		 END IFE
     ...
     &			   INDESCRIP)
		 ...
		 CALL RESPOND_MAIL('SYS$LOGIN:BULL.SCR',INFROM,O
     &			   INDESCRIP,STATUS)	
	      END IF 
	   END IF
	END IF 
	IF (IER.NE.0) THENE
	   WRITE (6,'('' ERROR: No message added.'')')n
	   IF (.NOT.STATUS) THEN 
...
900	IF (FILESPEC) CLOSE (UNIT=4)
	CALL LIB$DELETE_FILE('SYS$LOGIN:BULL.SCR;*')m
...
C)
	...
	CHARACTER*128 BULL_SIGNATUREI
	DATA BULL_SIGNATURE /'SYS$LOGIN:BULL_SIGNATURE.TXT'/M

	CHARACTER*256 INPUT

	IF (CLI$PRESENT('NOSIGNATURE')) RETURNE
	...
	IF (IER.NE.0) THENR
	   OPEN (UNIT=4,FILE='MX_SIGNATURE',STATUS='OLD',READONLY,I
     &		    SHARED,IOSTAT=IER,FORM='FORMATTED')
	END IF 
	...
	MATCH = .FALSE.
	DO WHILE (IER.EQ.0)
	   READ (4,'(A)',IOSTAT=IER) INPUTe
	   ILEN = TRIM(INPUT)
	   DO WHILE (.NOT.MATCH.AND.STREQ(INPUT(:6),'START ').AND.IER.EQ.0)
	      MATCH = STREQ(INPUT(7:ILEN),FOLDER_NAME(:TRIM(FOLDER_NAME)))N
	   ...
	   IF (IER.EQ.0) THEN
	      ...
	      ELSEF
	         ...
	         WRITE (3,'(A)',IOSTAT=IER) INPUT(:ILEN)A
	      END IFE
	   END IF
	END DOL
...
Cf
C  FUNCTION: Sends mail to address.P
CP
	...
	CHARACTER MAILER*128,INDESCRIP*(LINE_LENGTH)'

	INDESCRIP = SUBJECT
	LENDES = TRIM(INDESCRIP) 
	I = 1		! Must change all " to "" in SUBJECT field
	DO WHILE (I.LE.LENDES)t
	   IF (INDESCRIP(I:I).EQ.'"') THENF
	      IF (LENDES.EQ.LINE_LENGTH) THEN
		 INDESCRIP(I:I) = '`'(
	      ELSEC
		 INDESCRIP = INDESCRIP(:I)//'"'H
     &				//INDESCRIP(I+1:)
		 I = I + 1
		 LENDES = LENDES + 1
	      END IFS
	...
	VMSSERV = INDEX(FOLDER_DESCRIP,'VMSSERV').GT.0D

	IF (LISTSERV.OR.VMSSERV) CALL SETUSER(FOLDER_BBOARD)O

	IF (LISTSERV) THEN_
	   IF (SYS_TRNLNM('MX_NODE_NAME',MAILER)) THENG
	      REPLY_TO = .NOT.SYS_TRNLNM('MX_REPLY_TO',MAILER)N
	      IF (REPLY_TO) IER = LIB$SET_LOGICAL
     &			('MX_REPLY_TO',USERNAME(:TRIM(USERNAME)))
	   ELSE
	      REPLY_TO = .NOT.SYS_TRNLNM('PMDF_REPLY_TO',MAILER)2
	      IF (REPLY_TO) IER = LIB$SET_LOGICAL
	...
	IF (SYS_TRNLNM('BULL_MAILER',MAILER)) THENF
	   IF (LISTSERV) THEN
	...
	ELSE_
	   ...
	   DO WHILE (I.LT.LEN_SENDTO-2)
		 ...
		 I = I + 3
	      ELSE IF (SENDTO(I:I+1).EQ.'""') THENA
		 SENDTO(I:) = ''''//SENDTO(I+2:)
                 LEN_SENDTO = LEN_SENDTO - 1
              ELSE IF (SENDTO(I:I).EQ.'\'.AND.
     &                 SENDTO(I+1:I+1).NE.'\') THENF
		 SENDTO(I+1:) = '\'//SENDTO(I+1:)E
		 LEN_SENDTO = LEN_SENDTO + 1
		 I = I + 1
	      ELSE IF (SENDTO(I:I).EQ.''''.AND.
     &		       INDEX(SENDTO,'@').GT.I) THENE
     ...
     &                INDESCRIP(:LENDES),STATUS)
           IF (.NOT.STATUS) CALL SYS_GETMSG(STATUS)a
Cm
...
C     &       '" /SUBJECT="'//INDESCRIP(:LENDES)//'"',,,,,,STATUS)
	...
	IF (LISTSERV) THENR
	   ...
	   IF (REPLY_TO) IER = LIB$DELETE_LOGICAL('MX_REPLY_TO')D
	END IF.
...
C
C  FUNCTION: CHANGE command subroutine.z
C
	...
	CHARACTER INDESCRIP*(INPUT_LENGTH),INFROM*(INPUT_LENGTH)A
	CHARACTER*4 ANSWERY
	...
	IF (REMOTE_SET.GE.3) THEN
	   WRITE (6,'('' Cannot CHANGE messages in this folder.'')')E
	   RETURN
	END IF'
...
Ct
	...
	IF (CLI$PRESENT('SYSTEM')) THEN
	   IF (.NOT.SETPRV_PRIV()) THEN
	    WRITE (6,'(
     &	     '' ERROR: Not enough privileges to change to system.'')')F
	    RETURNY
	   ELSE IF (.NOT.BTEST(FOLDER_FLAG,2).AND.FOLDER_NUMBER.NE.0) THEN
	    WRITE (6,'(
     &       '' ERROR: /SYSTEM cannot be set with selected folder.'')')T
	    RETURNn
	   END IF
	END IFr
	...
	IF (CLI$PRESENT('PERMANENT').AND.
     ...
     &		FOLDER_ACCESS(USERNAME,FOLDER_FLAG,FOLDER_OWNER)) THEN
	   WRITE (6,'(L
     &	    '' ERROR: Not enough privileges to change to permanent.'')')u
	   RETURN
	END IFe
Cl
...
CO
	...
	CALL OPEN_BULLDIR_SHARED1

	SAME_OWNER = .TRUE.
	DO I=SBULL,EBULLe
	   CALL READDIR(I,IER)	! Get info for specified messages
	   IF (USERNAME.NE.FROM) SAME_OWNER = .FALSE. 1
	END DO
	...
	IF (.NOT.SAME_OWNER) THEN	! If doesn't match owner of bulletin,
     ...
     &		.AND.FOLDER_SET)) THEN				! folder owner?t
...
CL
C  If no switches were given, replace the full bulletin_
CE
	...
	IF ((.NOT.CLI$PRESENT('EXPIRATION')).AND.
     ...
     &	   (.NOT.CLI$PRESENT('SYSTEM')).AND.e
     &	   (.NOT.CLI$PRESENT('HEADER')).AND.i
     &	   (.NOT.CLI$PRESENT('SUBJECT')).AND.
     &	   (.NOT.CLI$PRESENT('TEXT')).AND.O
	...
	IF (SBULL.NE.EBULL.AND.(DOALL.OR.TEXT)) THEN_
	   WRITE (6,'('' ERROR: Cannot change text when replacing'',D
     &		      '' more than one messsage.'')')D
	   RETURN
	END IF 
	...
	IF (CLI$PRESENT('EXPIRATION').OR.DOALL) THEN/
	   SYSTEM = 0
	   CALL GET_EXPIRED(INPUT,IER)	
	   PERMANENT = BTEST(SYSTEM,1)r
	   IF (.NOT.IER) GO TO 910E
	   INEXDATE = INPUT(:11))
	   INEXTIME = INPUT(13:23)
	END IFR

8	LENDES = 0
	IF (CLI$PRESENT('HEADER').OR.DOALL) THENN
	   WRITE(6,1050)			! Request header for bulletinS
	...
	IF (LENDES.GT.0) THEN
	   ...
	   LENDES = MIN(LENDES+6,LEN(INDESCRIP))O
	END IFL
	...
	DO NUMBER=SBULL,EBULL
	 ...
	 IF (SBULL.NE.EBULL) THEN
	   ...
	   IF (IER.NE.NUMBER_PARAM+1) THEN	! Couldn't find messageS
	      CALL CLOSE_BULLDIRL
	 ...
	 IF (LENDES.GT.0.OR.TEXT.OR.DOALL) THEN
	   OPEN(UNIT=3,FILE='SYS$LOGIN:BULL.SCR',IOSTAT=IER,a
     &	     RECL=LINE_LENGTH,STATUS='SCRATCH',CARRIAGECONTROL='LIST')n
	   ...
	   CALL GET_BULL_LINE(BLOCK,LENGTH,INPUT,ILEN)_
	   IF (ILEN.GT.0.AND.INPUT(:6).EQ.'From: ') THENC
	      INFROM = INPUT(:ILEN)
	      LENFROM = ILENR
	      CALL GET_BULL_LINE(BLOCK,LENGTH,INPUT,ILEN)
	   END IF
	   ...
	   DO WHILE (ILEN.GT.0)		! Copy bulletin into fileT
	      WRITE (3,'(A)') INPUT(:ILEN)
	      CALL GET_BULL_LINE(BLOCK,LENGTH,INPUT,ILEN)
	   END DO
...
C 
     ...
     &	    ((CLI$PRESENT('EDIT').OR.EDIT_DEFAULT).AND.	! or /EDIT specifiedE
     &       (CLI$PRESENT('EDIT').NE.%LOC(CLI$_NEGATED)))) THEN 

	   IF ((CLI$PRESENT('EDIT').OR.EDIT_DEFAULT).AND. ! If /EDIT specifiedE
     &       (CLI$PRESENT('EDIT').NE.%LOC(CLI$_NEGATED))) THEN
     ...
     &		       ERR=920,FORM='FORMATTED',CARRIAGECONTROL='LIST') 
	            CALL OPEN_BULLFIL_SHARED	! Prepare to copy messageD
		    ILEN = LINE_LENGTH + 1
		    ...
		    IF (ILEN.GT.0.AND.INPUT(:6).EQ.'Subj: ') THEN)
		       CALL GET_BULL_LINE(BLOCK,LENGTH,INPUT,ILEN)
		    END IF
		    DO WHILE (ILEN.GT.0)	! Copy message into filef
		       WRITE (3,'(A)') INPUT(:ILEN).
		       CALL GET_BULL_LINE(BLOCK,LENGTH,INPUT,ILEN)
		    END DO
		    CALL CLOSE_BULLFIL
		 ...
		 CALL MAILEDIT('SYS$LOGIN:BULL.SCR',' ')
	      ELSE 
	         CALL DISABLE_PRIVS
		 CALL MAILEDIT(BULL_PARAMETER(:LEN_P),'SYS$LOGIN:BULL.SCR')G
	      END IF6
     ...
     &		READONLY,SHARED,ERR=920,FORM='FORMATTED') ! Try opening the file
	   ...
	   DO WHILE(1)				! Read until end of file to
		 ...
		 LAST_NOBLANK = ICOUNT
	      ELSE IF (ILEN.EQ.0) THENS
		 IF (ICOUNT.GT.0) THEN
		    ICOUNT = ICOUNT + 2		! COPY_BULL writes a line withT
     ...
     &			''. Reinput::'')') LINE_LENGTHE
	      ELSE IF (ILEN.GT.0) THEN		! If good input line enteredN
		 ICOUNT = ICOUNT + 1 + ILEN	! Increment character countU
		 WRITE(3,'(A)') INPUT(:ILEN)	! Save line in scratch file
		 LAST_NOBLANK = ICOUNT
	      ELSE IF (ILEN.EQ.0.AND.ICOUNT.GT.0) THENR
		 WRITE(3,'(A)') INPUT(:ILEN)	! Save line in scratch file
...
CH
C  Add bulletin to bulletin file and directory entry for to directory file.T
C0
	 ...
	 IF (SBULL.EQ.EBULL) THEN
	  ...
	  IF (IER.NE.NUMBER_PARAM+1.OR.DATE.NE.DATE_SAVE.OR.T
     ...
     &	    (DATE.NE.DATE_SAVE.OR.TIME.NE.TIME_SAVE.OR.DESCRIP.NE.INPUT))
	      ...
	      CALL READDIR(NUMBER_PARAM,IER)M
	   END DO

	   IF (IER.NE.NUMBER_PARAM+1) THEN	! Couldn't find messageM
	      CALL CLOSE_BULLDIR:
	 ...
	 IF (REC1.GT.0) THEN			! If text has been replaced 
	   ...
	   CALL OPEN_BULLFIL			! Prepare to add bulletinM
	   ...
	   IF (LENFROM.GT.0) THEN
	      CALL STORE_BULL(LENFROM,INFROM(:LENFROM),OBLOCK)T
	   END IF
	   IF (LENDES.GT.0) THEN 
     ...
     &		.NOT.PERMANENT,CLI$PRESENT('PERMANENT').OR.PERMANENT,L
     &		CLI$PRESENT('SHUTDOWN'),INEXDATE,INEXTIME)
	   IF (CLI$PRESENT('SYSTEM')) THENC
	      SYSTEM = IBSET(SYSTEM,0)T
	   ELSE IF (CLI$PRESENT('GENERAL')) THEN
	      SYSTEM = IBCLR(SYSTEM,0)
	   END IF
	   CALL WRITEDIR(NUMBER_PARAM,IER)T
	 ELSE
	   MSGTYPE = 0 
	   IF (CLI$PRESENT('SYSTEM').OR.(
     &		(BTEST(SYSTEM,0).AND..NOT.CLI$PRESENT('GENERAL'))) THENF
	      MSGTYPE = IBSET(MSGTYPE,0) 
	   END IF
	   IF (CLI$PRESENT('PERMANENT').OR.PERMANENT) THENI
     ...
     &		    .AND..NOT.PERMANENT) THEN
	   ...
	   END IF
	   IF (LENDES.EQ.0.AND..NOT.DOALL) INDESCRIP(7:) = DESCRIPD
	   IF (CLI$PRESENT('EXPIRATION').OR.DOALL) THEN
	      EXDATE = INEXDATE
	      EXTIME = INEXTIME
	   END IF
	   WRITE (REMOTE_UNIT,'(7A)',IOSTAT=IER)f
...
1000	FORMAT (' Enter message: End with ctrl-z, cancel with ctrl-c')T
1005	FORMAT (' ERROR: You are not reading any message.')
1010	FORMAT (' No message was replaced.')H
...
1090	FORMAT(' ERROR: Specified message is not owned by you.')F
1100	FORMAT(' Message(s) is not owned by you.',c
...
2020	FORMAT(1X,A)N
	...
	IF (EXPIRE) THENS
	   ...
	   IF (DIFF.LT.0) THEN			! If it's oldest expiration bull
	      NEWEST_EXDATE = EXDATE		! Update the header ine
	      NEWEST_EXTIME = EXTIME		! the directory filet
	      CALL WRITEDIR(0,IER)R
	   END IF
	ELSE IF (PERM.AND.(.NOT.BTEST(SYSTEM,1))) THEN!
	   IF (BTEST(SYSTEM,2)) THEN	
	      SYSTEM = IBCLR(SYSTEM,2)a
	      SHUTDOWN = SHUTDOWN - 1
	      CALL WRITEDIR(0,IER)s
	   END IF
	...
	ELSE IF (SHUT.AND.(.NOT.BTEST(SYSTEM,2))) THEN=
	   ...
	   EXDATE = '5-NOV-2000'A
	   NODE_AREA = 0	
	   IF (INCMD(:4).EQ.'REPL') THENO
	      IF (CLI$GET_VALUE('SHUTDOWN',NODE_NAME)
     &		    .NE.%LOC(CLI$_ABSENT)) THENT
		 CALL GET_NODE_NUMBER_OTHER(NODE_NUMBER,NODE_AREA,NODE_NAME)
	         IF (NODE_AREA.EQ.0) THEN
		    WRITE (6,'('' ERROR: Shutdown node name ignored.'',E
     &		               '' Invalid node name specified.'')')N
		 END IFY
	      END IFR
	   END IF
	   IF (NODE_AREA.EQ.0) CALL GET_NODE_NUMBER(NODE_NUMBER,NODE_AREA)Q
	   WRITE (EXTIME,'(I4)') NODE_NUMBER 
	   WRITE (EXTIME(7:),'(I4)') NODE_AREA 
	   DO I=1,11 
	      IF (EXTIME(I:I).EQ.' ') EXTIME(I:I) = '0'
	   END DO
	   EXTIME = EXTIME(1:2)//':'//EXTIME(3:4)//':'//M
...
C 
	...
	DIMENSION SEARCH_LEN(10)E

	EXTERNAL CLI$_ABSENT)

	NFOLDER = 1

	IF (CLI$PRESENT('SELECT_FOLDER')) THENi
	...
	DO WHILE (CLI$GET_VALUE('SELECT_FOLDER',FOLDER1_NAME)
     &	    .NE.%LOC(CLI$_ABSENT))		! Get the specified folders
	   IF (TRIM(FOLDER1_NAME).EQ.0) FOLDER1_NAME = FOLDER_NAME_
	   NFOLDER = NFOLDER + 1O
	   CALL WRITE_QUEUE(%VAL(SCRATCH_F),SCRATCH_F,FOLDER1_NAME)
	END DOE

	IF (CLI$PRESENT('SELECT_FOLDER')) SCRATCH_F = SCRATCH_F1N
	...
	IF (CLI$PRESENT('NOREPLIES')) THENP
	   SEARCH_STRING = 'RE:',
	   SEARCH_LEN(1) = 3D
	   SEARCH_NUM = 1
	...
	ELSE IF (CLI$PRESENT('SEARCH_STRING')) THEN
     ...
     &	    SEARCH_LEN(SEARCH_NUM)).NE.%LOC(CLI$_ABSENT))
	   ...
	   END DO
           IF (SEARCH_NUM.GT.1) SEARCH_NUM = SEARCH_NUM - 1T
	ELSEN
	   SEARCH_STRING = ' ' 
	END IFT
	...
	DO WHILE (NFOLDER.GT.0.AND.FOUND.LE.0)
	   IF (.NOT.CLI$PRESENT('SELECT_FOLDER').OR.N
     &		SCRATCH_F.NE.SCRATCH_F1) D
     &	      CALL GET_SEARCH(FOUND,SEARCH_STRING,SEARCH_NUM,
     &		SEARCH_LEN,MATCH_MODE,START_BULL, 
     &		CLI$PRESENT('REVERSE'),CLI$PRESENT('SUBJECT')A
     &	        .OR.CLI$PRESENT('NOREPLIES'),
     &		CLI$PRESENT('REPLY'),.TRUE.,CLI$PRESENT('START'),U
     &		CLI$PRESENT('FROM'),CLI$PRESENT('NEGATED')
     &	        .OR.CLI$PRESENT('NOREPLIES'),MATCH_MODE)
	   IF (FOUND.EQ.-1) THENR
	      NFOLDER = 0
	   ELSE IF (FOUND.LE.0) THEN,
	      IF (.NOT.CLI$PRESENT('SELECT_FOLDER').OR.
     &		   SCRATCH_F.NE.SCRATCH_F1) NFOLDER = NFOLDER - 1I
	      IF (NFOLDER.GT.0) THENU
...
C"
	...
	IF (.NOT.IER1.AND..NOT.REPLY.AND.
     &      (SUBJECT.OR.SEARCH_MODE.NE.1)) THEN 
	   ...
	   IF (TRIM(SAVE_STRING).EQ.0) THEN
	      WRITE (6,'('' No search string present.'')')E
	...
	ELSE IF (REPLY) THEN 
	   ...
	   IF (START_BULL+1.NE.IER) THENC
	      WRITE (6,'('' ERROR: No message being read.'')')P
	   ...
	   ELSE
	      ...
	      IF (REVERSE) START_BULL = START_BULL - 2D
	   END IF
	END IF,

	SAVE_STRING = SEARCH_STRING

	CALL STR$UPCASE(SEARCH_STRING,SEARCH_STRING)	! Make upper caseM

	IF (IER1.OR.SEARCH_MODE.NE.OLD_SEARCH_MODE.OR.G
     &	    MATCH_MODE.NE.OLD_MATCH_MODE.OR.REVERSE.OR.REPLY) THENC
	   ...
	   END IF
	   IF (REVERSE) THENQ
	      START_BULL = MIN(START_BULL,NBULL-1)F
	      END_BULL = 1E
	      STEP_BULL = -1b
	   ELSE
	      END_BULL = NBULLT
	...
	SAVE_BULL_SEARCH = 0L
	BULL_SEARCH = START_BULL+1L
	DO WHILE ((STEP_BULL.EQ.1.AND.BULL_SEARCH.LE.END_BULL).OR.E
     ...
     &			BULL_SEARCH,DUMMY)L
	   ...
	   IF (IER.EQ.BULL_SEARCH+1.AND.SEARCH_MODE.NE.2) THENS
	      IF (SEARCH_MODE.EQ.4) THENN
	         CALL STR$UPCASE(DESCRIP1,FROM)
	      ELSEY
	         CALL STR$UPCASE(DESCRIP1,DESCRIP)T
	      END IF
     ...
     &		   DESCRIP1(:4).EQ.'RE: ')))) THEN
		    ...
		    GO TO 900 
	         END IF    
	      ELSE IF (FLAG.EQ.1) THENR
		 WRITE (6,'('' Search aborted.'')')E
		 GO TO 900
              ELSE IF (NEGATED) THEN h
                 FOUND = BULL_SEARCH
                 GO TO 900
	      END IF 
	   END IF
	   IF (IER.EQ.BULL_SEARCH+1.AND.SEARCH_MODE.EQ.2) THEN
		 ...
		 CALL REMOTE_READ_MESSAGE(BULL_SEARCH,IER)
		    ...
		    IF (IER.GT.0) GO TO 900e
	         END IF
	      END IFs
	      ILEN = LINE_LENGTH + 1i
              MATCHES = 0b
	      DO I=1,SEARCH_NUM
		 MATCH_FOUND(I) = .FALSE.E
	      END DO 
	      DO WHILE (ILEN.GT.0)D
	         CALL GET_BULL_LINE(BLOCK,LENGTH,INPUT,ILEN)S
     ...
     &	             (MATCHES.EQ.SEARCH_NUM.AND.MATCH_MODE.EQ.1)) THEN1
		    FOUND = BULL_SEARCH 
...
900	IF (FOUND.LE.0.AND.REMOTE_SET.EQ.3) THEN
	   ...
	   END IF
	END IF.
	IF (FILES) CALL CLOSE_BULLFIL		! End of bulletin file readn
	IF (FILES) CALL CLOSE_BULLDIR
	CALL CANCEL_CTRLC_AST
	CALL ENABLE_CTRL.

	NEXT = .FALSE.
	IF (STEP_BULL.EQ.-1) INCMD(:4) = SAVECMDC
...
CC
C  FUNCTION: Undeletes deleted message.L
C 
...
5	   FORMAT(I<LEN_P>) 
	...
	ELSEA
	   BULL_DELETE = BULL_POINT	! Delete the file we are readingN
	END IF6
...
C.
	...
	IF (USERNAME.NE.FROM) THEN	! If doesn't match owner of bulletin,)
     ...
     &		.AND.FOLDER_SET)) THEN				! folder owner?
	      WRITE(6,1040)		! Then error out.r
	      GO TO 100
	   ELSE
	      CALL READDIR(BULL_DELETE,IER) ! Get info for specified bulletin
	      IF (IER.NE.BULL_DELETE+1) THEN	! Was bulletin found?n
	...
	IF (.NOT.REMOTE_SET) THEN
	   ...
	   WRITE (6,'('' Message was undeleted.'')')U
	ELSEI
...
1040	FORMAT(' ERROR: Message was not undeleted. Not owned by you.')$
	...
	INCLUDE 'BULLNEWS.INC'S

	CHARACTER*20 MAIL_PROTOCOLI
	...
	IF (INDEX(INPUT,'<').GT.0.AND.	! Name may be of formN
     &		INDEX(INPUT,'@').GT.INDEX(INPUT,'<')) THEN
	   INPUT = INPUT(INDEX(INPUT,'<'):)! personal-name <net-name>
	END IFS
	...
	IF (LMAIL.EQ.0) THENT
	   ...
	   END IF
	   LMAIL = TRIM(MAIL_PROTOCOL)o
	   IF (LMAIL.GT.0.AND.MAIL_PROTOCOL(LMAIL:LMAIL).NE.'%') THEN
	      MAIL_PROTOCOL = MAIL_PROTOCOL(:LMAIL)//'%'C
	      LMAIL = LMAIL + 1
	   END IF
	   IF (LMAIL.EQ.0) THEN
	      LMAIL = -1t
	...
	INPUT = MAIL_PROTOCOL(:LMAIL)//'"'//INPUT(:TRIM(INPUT))//'"'w

	IF (ILEN.NE.0) ILEN = ILEN + LMAIL + 2r
