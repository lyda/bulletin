C
C  BULLETIN1.FOR, Version 3/23/94
...
C
	...
	CALL DISABLE_PRIVS
	CALL SENDMAIL('SYS$LOGIN:BULL.SCR',BULL_PARAMETER(:LEN_P)
     &			,SUBJECT_LINE,STATUS)
C       CALL LIB$SPAWN('$MAIL SYS$LOGIN:BULL.SCR '//BULL_PARAMETER(:LEN_P)
...
C
	...
	CHARACTER PASSWORD*32,DEFAULT_USER*12
...
C
	...
	CHARACTER SAVE_FOLDER*44,POST_SUBJECT*256
	...
	POST_NEWS = (.NOT.ORIGINAL.AND.REMOTE_SET.EQ.4).OR.REMOTE_SET.EQ.3
	...
	ELSE IF (READ_ONLY.OR.(MERGE.AND.REMOTE_SET.GT.0)) THEN
	   ...
	   END IF
	   IER1 = .FALSE.
	ELSE IF (REMOTE_SET.EQ.0.AND.NEWS_FEED()) THEN
...
C
	...
	DO WHILE (NUM_COPY.GT.0.AND.IER.EQ.0)
     ...
     &				F_EXPIRE_LIMIT
	   ...
	   IF (REMOTE_SET.GE.3.AND..NOT.ORIGINAL) THEN
	      ILEN = LINE_LENGTH + 1
...
100	   CONTINUE
	...
	IF (FOLDER_NUMBER.GE.0.AND.IER.EQ.0.AND..NOT.POST_NEWS
     ...
     &			       F_NEWEST_BTIM)
	   CALL UPDATE_FOLDER			! Update folder infoi
Cs
C  If user is adding message, an no new messages, update last read time foro
...
Cs
	...
	IF (IER.EQ.0) THEN
	   WRITE (6,'('' Successful copy to folder '',A)')F
     &		FOLDER(:TRIM(FOLDER))//'.'
	   IF (MERGE) THENC
	      CALL LIB$DELETE_FILE(FOLDER_FILE(:TRIM(FOLDER_FILE))//P
     &		  '.BULLDIR;-1')
	   END IF
	ELSE IF (MERGE) THEN 
	   WRITE (6,'('' ERROR: Copy aborted. No files copied.'')')
	ELSEC
	   WRITE (6,'('' ERROR: Copy aborted. '',I,'' files copied.'')')2
     &			BULL_POINT - START_BULL_POINT
	END IF

	IF (.NOT.POST_NEWS) HEADER = SAVE_HEADERS
	IF (ORIGINAL.AND.(NEWS_FEED().OR.REMOTE_SET.EQ.4)) RETURN
...
C*
	...
	IF (CLI$PRESENT('NOW').AND..NOT.FIRST.AND.T
     &	    INCMD(:4).EQ.'PRIN') THEN
	   WRITE (6,'('' Printing all previously queued messages.'')')T
	   GO TO 200&
	ELSE IF (.NOT.FIRST) THEN
	   ...
	   IER = CLI$GET_VALUE('FORM',TEST,FLEN)D
	   CHANGED = CHANGED.OR.(TEST(:TLEN).NE.FORM(:FLEN).AND.TLEN.GT.0)E
	   IF (CHANGED) THEN 
...
50	IF (PRINT_NUM.EQ.0) THEN'
	   IER = CLI$GET_VALUE('BULLETIN_NUMBER',BULL_PARAMETER,LEN_P)E
	...
	HEAD = CLI$PRESENT('HEADER')R

	DO I=SBULL,EBULL
	   I1 = I
	   CALL READDIR(I,IER)		! Get info for specified messageA
	   IF (IER.NE.I+1.OR.I.GT.EBULL.OR.(.NOT.CLI$PRESENTR
     &		('ALL').AND.I1.EQ.SBULL.AND.I.NE.SBULL)) THENP
	      IF (REMOTE_SET.NE.3.OR.I1.EQ.SBULL) WRITE(6,1030) I1 
	      IF (I1.GT.SBULL) GO TO 1002
	   ...
	   IF (ILEN.GT.0.AND.INPUT(:6).EQ.'From: ') THEN 
	      IF (HEAD) THEN/
		 WRITE(24,1060) INPUT(7:ILEN),DATE//' '//TIME(:8)S
	      END IF_
	      CALL GET_BULL_LINE(BLOCK,LENGTH,INPUT,ILEN)
	   ELSE IF (HEAD) THENC
	   ...
	   ELSE
	      ...
	      IF (ILEN.GT.0) WRITE (24,'(A)') INPUT(:ILEN)L
	   END IF

	   DO WHILE (ILEN.GT.0)		! Copy bulletin into fileD
	      CALL GET_BULL_LINE(BLOCK,LENGTH,INPUT,ILEN)
...
100	IF (PRINT_NUM.EQ.0) THEN
     ...
     &		 BULL_PARAMETER(FIRST_ALPHA(BULL_PARAMETER):).
	   ELSE
	      WRITE(6,1045)
     &		 BULL_PARAMETER(FIRST_ALPHA(BULL_PARAMETER):)N
	      IER = OTS$CVT_L_TI(EBULL,BULL_PARAMETER,,,)
	      WRITE(6,1046)
     &		 BULL_PARAMETER(FIRST_ALPHA(BULL_PARAMETER):)L
	   END IF
...
200	IF (FIRST) RETURN
	...
	CALL ADD_2_ITMLST(18,SJC$_FILE_SPECIFICATION,
     &		%LOC('SYS$LOGIN:BULL.LIS'))e

	CALL ADD_2_ITMLST(QLEN,SJC$_QUEUE,%LOC(QUEUE))H
	...
	IER=SYS$SNDJBCW(,%VAL(SJC$_ENTER_FILE),,%VAL(SJC_ITMLST),IOSB,,)1
	...
	ELSES
	   ...
	   IF (IER) WRITE (6,'('' Job BULL (queue '',A,'', entry '',A,o
...
1015	FORMAT(' ERROR: Specified message number has incorrect format:')P
...
CO
	...
	COMMON /NEXT/ NEXT 
	LOGICAL NEXT /.FALSE./H

	COMMON /POST/ POSTTIMEL

	COMMON /MSGID/ MESSAGE_ID
	CHARACTER*256 MESSAGE_IDE

	COMMON /BULL_USER_CUSTOM/ BULL_USER_CUSTOM 
	DATA BULL_USER_CUSTOM/.FALSE./S

	COMMON /MAIN_HEADER_INFO/ INFROM,INDESCRIP,LEN_FROM,LEN_DESCRPF
	...
	BULL_USER_CUSTOM = IBCLR(BULL_USER_CUSTOM,3)R
	...
	IF (READ_COUNT.GT.0) GO TO 100		! Skip init steps if this isQ
	...
	IF (INCMD(:4).EQ.'READ') THEN		! If READ command...
     ...
     &		    CLI$PRESENT('SEEN').EQ.%LOC(CLI$_NEGATED)) THEN
	   ...
	   END IF
	   IF (READ_TAG) THEN
	      IF (.NOT.(FOLDER_NUMBER.GE.0.OR.REMOTE_SET.EQ.3)) THENV
		 WRITE (6,'('' ERROR: Invalid qualifier'',
     &			    '' with remote folder.'')')
		 READ_TAG = 1 + IBSET(0,2) + IBSET(0,3)I
		 GO TO 9999T
	      END IFN
	   ...
	   IF (CLI$PRESENT('SINCE')) THEN		! was /SINCE specified?
		 ...
		 CALL GET_MSGKEY(TODAY,MSG_KEY) 
	      ELSEe
		 CALL SYS_BINTIM(DATETIME,MSG_BTIM)u
		 CALL GET_MSGKEY(MSG_BTIM,MSG_KEY)
	      END IF 
	      CALL OPEN_BULLDIR_SHAREDL
	      CALL READDIR_KEYGE(IER)
	      CALL CLOSE_BULLDIRe
	   ELSE IF (CLI$PRESENT('NEW')) THEN	! was /NEW specified?L
	      NEW = .TRUE.P
	      IF (REMOTE_SET.LT.3) THEN
		 DIFF = COMPARE_BTIM(LAST_READ_BTIM(1,FOLDER_NUMBER+1),S
     &				 F_NEWEST_BTIM)N
		 IF (DIFF.GE.0) THEN
     ...
     &			.OR.(BULL_TAG.AND.BTEST(BULL_TAG,1)))) THEN
		       ...
		       CALL GET_THIS_TAG(FOLDER_NUMBER,IER1,IER,DUMMY)
		       IF (IER1.EQ.0) THEN
	                  CALL NEWS_UPDATE_NEWEST_MESSAGE(IER)I
			  IER = 0
	               END IF
		    ELSE IF (IER.EQ.0) THENN
		       WRITE (6,'('' No new messages are present.'')')
		       GO TO 9999
	            END IF_
	         END DO
	...
	NEXT = .FALSE.=
	IF (INCMD(:1).EQ.'N'.OR.INCMD.EQ.' ') THENC
	   NEXT = .TRUE. 
	ELSE IF (INCMD(:4).EQ.'READ') THENI
...
50	IF (READ_TAG) THEN 
     ...
     &	       (INCMD(:4).EQ.'LAST'.AND.BTEST(READ_TAG,3))) THENP
	      IF (BULL_NOW.EQ.0.OR.INCMD(:4).EQ.'LAST') THENI
		 MSG_NUM = F_NBULL+1
	      ELSEI
		 MSG_NUM = BULL_NOWL
	      END IF 
     ...
     &				   (FOLDER_NUMBER,IER1,BULL_READ,DUMMY) 
		 ...
		 IF (IER1.EQ.0) IER = BULL_READ + 1I
	      END IFE
	      DO WHILE (IER1.EQ.0) 
		 CALL GET_NEXT_TAG(FOLDER_NUMBER,IER1,BULL_READ,DUMMY)
		 IF (IER1.EQ.0) IER = BULL_READ + 1F
	      END DO1
	      CALL CLOSE_BULLDIRI
	   ELSE IF (INCMD(:4).EQ.'FIRS') THEN
	      ...
	      CALL GET_NEXT_TAG(FOLDER_NUMBER,IER1,BULL_READ,DUMMY)
	      IF (IER1.EQ.0) IER = BULL_READ + 1'
	   ELSE IF (NEXT.OR.SINCE.OR.NEW) THENl
	      OLD_NEXT = NEXT
	      NEXT = .FALSE.(
	...
	IF (.NOT.SINCE.AND.(.NOT.READ_TAG.OR.(.NOT.NEXT.AND.i
     &		INCMD(:4).NE.'LAST'.AND.INCMD(:4).NE.'BACK'.AND.
     &		INCMD(:4).NE.'FIRS'))) THENE
	   IF (BULL_READ.GT.0) THEN		! Valid bulletin number?
	      CALL OPEN_BULLDIR_SHARED 
	      CALL READDIR(BULL_READ,IER)	! Get bulletin directory entryC
	      IF (IER.NE.BULL_READ+1.AND.REMOTE_SET.GE.3H
     &			   .AND.INCMD(:4).EQ.'READ') THEN
		 ...
		 END IFE
	      END IFE
	      IF (REMOTE_SET.LT.3.AND.I
     &			   READ_COUNT.EQ.-1.AND.IER.NE.BULL_READ+1) THENL
		 READ_COUNT = 0(
		 IF (IER.NE.BULL_READ+1) THENA
		    CALL READDIR(0,IER)N
		    IF (NBULL.GT.0) THEN
		       BULL_READ = NBULL
		       CALL READDIR(BULL_READ,IER)
		    ELSE
		    ...
		    END IF
		 END IF 
	      ELSE IF (READ_TAG.AND.IER.EQ.BULL_READ+1) THENU
	...
	IF (IER.NE.BULL_READ+1) THEN		! Was bulletin found?
	   IF (REMOTE_SET.LT.3) THENG
	      WRITE(6,1030)			! If not, then error outS
	   ELSE
	      WRITE(6,1040)
	   END IF
	   NEXT = .FALSE.
	...
	IF (BTEST(BULL_USER_CUSTOM,1)
     &			.AND.(NEXT.OR.INCMD(:4).EQ.'BACK'.OR.
     &		INCMD(:4).EQ.'LAST'.OR.INCMD(:4).EQ.'FIRS')) THEN 
	   ...
	   IF (.NOT.OK) THENR
	      BULL_NOW = MSG_NUM 
	      IF (INCMD(:4).EQ.'BACK'.OR.INCMD(:4).EQ.'LAST') THENI
	         BULL_READ = MSG_NUM - 1O
	      ELSER
	         BULL_READ = MSG_NUM + 1B
	      END IF'
	      IF (REMOTE_SET) CALL CLOSE_BULLFILR
	      IF (REMOTE_SET.LT.3) THEN
	...
	NEXT = .FALSE. 
	IF (REMOTE_SET.LT.3) THEN
           IF (INCMD(:4).NE.'SEAR') THEN
	      DIFF = COMPARE_BTIM(MSG_BTIM,LAST_READ_BTIM(1,FOLDER_NUMBER+1))
	      IF (DIFF.GT.0) THEN
	         CALL COPY2(LAST_READ_BTIM(1,FOLDER_NUMBER+1),MSG_BTIM)
	      END IF.
	   END IF
	   IF (BULL_TAG.AND.BTEST(BULL_TAG,1)) CALL ADD_TAG(IER,2)U
	   IF (INCMD.EQ.'LAST'.AND..NOT.READ_TAG) THEN_
	      CALL COPY2(LAST_READ_BTIM(1,FOLDER_NUMBER+1),F_NEWEST_BTIM)
	   END IF
	ELSEE
	   IF (REMOTE_SET.EQ.4) MESSAGE_ID = NEWS_MSGID
	   IF (BULL_NEWS_TAG) CALL ADD_TAG(IER,2)
	   IF (INCMD.EQ.'LAST'.AND..NOT.READ_TAG) THEND
	      CALL NEWS_UPDATE_NEWEST_MESSAGE(F_NBULL)P
	   END IF
	   IF (INCMD(:4).NE.'SEAR') THENT
	      CALL NEWS_UPDATE_NEWEST_MESSAGE(BULL_READ)X
	   ELSE
	      CALL NEWS_GET_NEWEST_MESSAGE(IER)
	      IF (IER.EQ.BULL_READ) CALL NEWS_UPDATE_NEWEST_MESSAGE(BULL_READ)I
	   END IF
	END IFI

	EDIT = .FALSE.O

	PAGE_WIDTH = REAL_PAGE_WIDTHT

	IF (INCMD(:1).NE.' '.AND.READIT.EQ.0) THENT
	   IF (CLI$PRESENT('EDIT')) THEN 
	      OPEN(UNIT=3,FILE='SYS$LOGIN:BULL.SCR',IOSTAT=IER,
     &		RECL=LINE_LENGTH,STATUS='NEW',CARRIAGECONTROL='LIST'),
	      IF (IER.NE.0) THEN 
	...
	IF (REMOTE_SET.GE.3) THEN
     ...
     &				BULL_POINT,F_START,F_NBULL
	   ...
	   END DO
	ELSED
	   WRITE (HEADLINE,'(1X,I,'' of '',I)') BULL_POINT,F_NBULLL
	END IF2
	...
	ELSE
	   WRITE(6,'(1X,A)') HEADLINE(:TRIM(HEADLINE))F
	END IFX

	END = 1					! Outputted 1 line to screenO

	IF (EXDATE(8:9).EQ.'18'.OR.INDEX(EXDATE,'1900').GT.0) THENs
	   IF (REMOTE_SET.NE.3) THENR
	      INPUT = 'Date:   '//DATE(:TRIM(DATE))//' '//TIME(:5),
     &				//'   (DELETED)'
	   ELSE
	      INPUT = 'Date:   '//DATE(:TRIM(DATE))//' '//TIME(:5) 
	...
	ELSE(
	   ...
	   ELSE
	      WRITE(6,'('' Subj:   '',A)') DESCRIP(:TRIM(DESCRIP))A
	   ...
	   END IF
	END IFP
	IF (LINE_OFFSET.EQ.0) ILEN = LINE_LENGTH + 1R
	CALL CLOSE_BULLFIL			! End of bulletin file readD

	IF (EDIT) GO TO 200

	WRITE(6,'(1X)')

	IF (READIT.GT.0) WRITE(6,'(1X)')E
	END = END + 1
C&
...
C  and thus causing the bulletin file to stay open.  The temporary memory_
C  is structured as a linked-list queue, where SCRATCH_B1 points to the header
C  of the queue.  See BULLSUBS.FOR for more description of the queue.u
Cs

	IF (SCRATCH_B1.NE.0) THEN		! Is queue empty?M
	   SCRATCH_B = SCRATCH_B1		! No, set queue pointer to headF
	...
	IF (READ_COUNT.EQ.-2) THEN		! Just output header first read
	   ...
	   GO TO 9999
	ELSE 
	   READ_COUNT = BLOCK			! Init bulletin record counterT
	END IFE
...
200	DISPLAY = 0'
	...
	DO WHILE (ILEN.GT.0.AND.MORE_LINES)
	   ...
	   ELSE IF (ILEN.GT.0) THEN
		 ...
		 ELSE.
		    CALL WRITE_QUEUE(%VAL(SCRATCH_B),SCRATCH_B,INPUT)C
		 END IF'
	      ELSET
		 CHAR_OFFSET = CHAR_OFFSET + PAGE_WIDTH'
		 IF (LEN_TEMP.LE.CHAR_OFFSET+PAGE_WIDTH-1) THENu
	...
	CALL CLOSE_BULLFIL			! End of bulletin file readN

	IF (EDIT) THEN 
	   CLOSE (UNIT=3)
	   CALL MAILEDIT('SYS$LOGIN:BULL.SCR',' ') 
	   CALL LIB$DELETE_FILE('SYS$LOGIN:BULL.SCR;*')
	   READ_COUNT = 0			! init bulletin record counter_
	   GO TO 9999
	END IF 

CE
C  Bulletin page is now in temporary memory, so output to terminal.C
C  Note that if this is a /READ, the first line will have problems withE
C  the usual FORMAT statement.  It will cause a blank line to be outputted
C  at the top of the screen.  This is because of the input QIO at thee
...
CI
	...
	IF (ILEN.EQ.0) THEN			! End of message?
	   READ_COUNT = 0			! init bulletin record counterY
	ELSE	! Possibly end of message since end of page could be last line
	   CALL TEST_MORE_RECORDS(BLOCK,LENGTH,IREC)U
	   IF (IREC.EQ.0) THEN			! Last record?
	      CALL TEST_MORE_LINES(ILEN)	! More lines to read?E
	      IF (ILEN.GT.0) THEN		! Yes, there are still moreI
		 IF (READIT.EQ.0) WRITE(6,1070)	! say there is more of bulletinI
...
9999	POSTTIME = .FALSE.S
	IF (KILL) BULL_USER_CUSTOM = IBSET(BULL_USER_CUSTOM,3)W
	RETURN3

1030	FORMAT(' No more messages.') 
1040	FORMAT(' Message not found.')
1070	FORMAT(1X,/,' Press RETURN for more...',/)_
...
2000	FORMAT(A)
	...
	DO I=1,LEN_TEMP
	   IF (INPUT(I:I).GE.'A'.AND.INPUT(I:I).LE.'Z') THENS
	...
	SUBROUTINE READNEW(REDO)8
C
C  SUBROUTINE READNEWC
C 
...
C0
	...
	CHARACTER INREAD*1:
	EQUIVALENCE (INREAD4,INREAD)4

	DATA LEN_FILE_DEF /0/, INREAD/0/H

	LOGICAL SLOW,SLOW_TERMINALR

	FIRST_MESSAGE = BULL_POINTM

	IF (ICHAR(INREAD).EQ.0) THEN	! If calling READNEW for first timeU
	   SLOW = SLOW_TERMINAL()	! Check baud rate of terminal
	END IF				! to avoid gobs of output

	LEN_P = 0			! Tells read subroutine there ist
					! no bulletin parameter

1	WRITE(6,1000)			! Ask if want to read new bulletinsL

	CALL GET_INPUT_NUM(NUMREAD,NLEN)	! Get inputE
	CALL STR$UPCASE(NUMREAD,NUMREAD)	! Make input upper caseW
	...
	IF (IER.NE.0) THENC
	   ...
	   IF (INREAD.EQ.'N'.OR.INREAD.EQ.'Q'.OR.INREAD.EQ.'E') THEN 
		 ...
		 WRITE (6,'(''+o'',$)')S
	      END IFS
	      RETURN	! If NO, exitL
	      		! Include QUIT to be consistent with next questionS
	   ELSE
	      CALL LIB$ERASE_PAGE(1,1)F
	   END IF
	END IFI
...
5	CALL READ_MSG(READ_COUNT,BULL_POINT+1)	! Read next bulletin
	BULL_POINT_READ = BULL_POINTS
	IF (READ_COUNT.EQ.0) THEN		! Is full bulletin displayed?E
	   CALL OPEN_BULLDIR_SHARED		! If so, see if more new bulls
10	   CALL READDIR(BULL_POINT+1,IER_POINT)
	   IF ((IER_POINT.EQ.BULL_POINT+2).AND.	! If system bulletin (and systemA
     &	       (SYSTEM.AND.BTEST(FOLDER_FLAG,2))) THEN	! folder) then skip it.n
	      BULL_POINT = BULL_POINT + 1
	      GO TO 10a
	   END IF
	   CALL CLOSE_BULLDIR
...
12	IF (READ_COUNT.EQ.0) THEN		! Prompt user in between
	...
	BLOCK_SAVE = BLOCK 
	LENGTH_SAVE = LENGTH.
	BULL_POINT_SAVE = BULL_POINTA

	IF (INREAD.EQ.'Q') THEN		! If Q , then QUIT
	   WRITE (6,'(''+Quit'',$)')L
	   RETURN
	ELSE IF (INREAD.EQ.'D') THEN	! If D , then redisplay directoryT
	   WRITE (6,'(''+Dir'',$)')
	   REDO = .TRUE..
	   RETURN
	ELSE IF (INREAD.EQ.'F'.AND..NOT.CAPTIVE(1)) THEN+
	   ...
	   IF (LEN_FILE_DEF.EQ.0) THEND
	      CALL LIB$SYS_TRNLOG('SYS$LOGIN',ILEN,FILE_DEF)T
	      IER = LIB$FIND_FILE(FILE_DEF//'BULL.DIR',
     &			BULL_PARAMETER,CONTEXT)
	      IF (IER) THEN
		 FILE_DEF = BULL_PARAMETER(:ILEN-1)//'.BULL]'R
		 LEN_FILE_DEF = ILEN + 5
	      ELSE.
		 FILE_DEF = 'SYS$LOGIN:'
		 LEN_FILE_DEF = 10
     ...
     &				BULL_PARAMETER(:LEN_P)
		 LEN_P = LEN_P + LEN_FILE_DEFE
	      END IF)
	   END IF

	   BULL_POINT = BULL_POINT_READ
	   INCMD = 'FILE '//BULL_PARAMETER(:LEN_P)E
	   IER = CLI$DCL_PARSE(INCMD,BULLETIN_SUBCOMMANDS) 
	   CALL FILE(0,.TRUE.) 
	   GO TO 11
	ELSE IF (INREAD.EQ.'P') THENI
	   WRITE (6,'(''+P'',$)')
	   BULL_POINT = BULL_POINT_READ
     ...
     &	       INDEX(FOLDER_DESCRIP,'<').GT.0) THEN
		 ...
		 WRITE (6,'(''+P'',$)')D
		 INCMD = 'REPLY'
	      ELSE IF (INREAD.EQ.'U') THENF
		 WRITE (6,'(''+U'',$)') 
		 INCMD = 'RESPOND'
	      ELSE IF (INREAD.EQ.'B') THENM
		 WRITE (6,'(''+B'',$)')M
		 INCMD = 'RESPOND/LIST' 
	      ELSE 
		 GO TO 11_
	   ...
	   ELSE IF (READ_ONLY) THEN
	      WRITE (6,'(
     &		 '' ERROR: You do not write access to this folder.'')')N
	   ELSE
	      INCMD = 'REPLY'
	      IER = CLI$DCL_PARSE(INCMD,BULLETIN_SUBCOMMANDS)
	      CALL REPLY 
...
20	   CALL READDIR(BULL_POINT+1,IER)
	   IF (IER.NE.BULL_POINT+2) THEN	! If no NEXT bulletinG
	      CALL CLOSE_BULLDIR		! Exit_
	      WRITE(6,1010)
	      RETURNN
	   ELSE IF (SYSTEM.AND.BTEST(FOLDER_FLAG,2)) THEN
	      BULL_POINT = BULL_POINT + 1	! If SYSTEM bulletin, skip it
	      GO TO 20			! Look for more bulletinsL
	   END IF
	   CALL CLOSE_BULLDIR
	ELSE IF (INREAD.EQ.'R') THEN 
	   ...
	   READ (NUMREAD,'(I<NLEN>)',IOSTAT=IER) TEMP_READ3
...
1000	FORMAT(' Read messages? Type N(No),E(Exit),message',E
     &	 ' number, or any other key for yes: ',$)
1010	FORMAT(' No more messages.')A
1020	FORMAT(1X,<PAGE_WIDTH>('-'),/,' Type Q(Quit),F(File),D(Dir),',X
     &	'R(Read msg #),P(Reply) or other for next message: ',$)
1030	FORMAT(1X,<PAGE_WIDTH>('-'),/,' Type Q(Quit),F(File),N(Next),',
     &	'D(Dir),R(Read msg #),P(Reply) or other for MORE: ',$)E
1040	FORMAT(' Type P to post reply, U to reply to user,',N
     &	' B to do both, or other to quit: ',$)L
...
C.
	...
	IF (FOLDER_ACCESS(USERNAME,FOLDER_FLAG,FOLDER_OWNER)) THEN:
	   IER = CLI$GET_VALUE('DEFAULT_EXPIRE',EXPIRE,EX_LEN)l
	   IF (EX_LEN.GT.3) EX_LEN = 3)
	   READ (EXPIRE,'(I<EX_LEN>)') TEMP

	   CALL OPEN_BULLFOLDER		! Open folder file
	   CALL READ_FOLDER_FILE_KEYNAME(FOLDER,IER)(
	   IF (TEMP.GT.BBEXPIRE_LIMIT.AND..NOT.SETPRV_PRIV()) THENa
	      WRITE (6,'('' ERROR: Expiration cannot be > '',
     &			I3,'' days.'')') BBEXPIRE_LIMIT
	   ELSE IF (TEMP.LT.-1) THENN
	      WRITE (6,'('' ERROR: Expiration must be > -1.'')')	
	   ELSE
	      FOLDER_BBEXPIRE = TEMPS
	...
	IF (SLIST.GT.0) THEN'
     ...
     &		  FOLDER_DESCRIP(I:I).NE.'%') THEN
		 ...
		 I = FLEN + 2N
	      END IF 
	   END DO
	   IF (I.EQ.FLEN+1) NEWS_FEED = .TRUE.C
	END IFI
